import time, random, glob, sqlite3, tkinter, tkinter.messagebox, math
...
def auswertung():
    ...
    if not glob.glob("gui_highscore.db"):
        ...
        cursor = con.cursor()
        sql = "CREATE TABLE IF NOT EXISTS daten(name TEXT, zeit REAL)"
        cursor.execute(sql)
        con.close()
    ...
    for dsatz in cursor:
        ...
        i = i+1
    tkinter.messagebox.showinfo("Highscores", ausgabe)
    con.close()
    main.destroy()
